...
export async function createProductionRun(values: ProductionRunFormValues): Promise<ActionResult<{ id: string }>> {
  ...
  try {
    const result = await db.insert(productionRuns).values({
      ...
      biocharDryWeightKg: values.biocharDryWeightKg ?? null,
      biocharWetWeightKg: values.biocharWetWeightKg ?? null,
      biocharDryMoisturePercent: values.biocharDryMoisturePercent ?? null,
      uncarbonizedBiocharKg: values.uncarbonizedBiocharKg ?? null,
